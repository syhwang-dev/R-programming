...
install.packages("remotes")  # 깃허브 설치를 위해 까는 것
remotes::install_github("bit2r/bitTA")  # 한국어 형태소 분석 / 깃허브(아이디/저장소명)
# remotes::install_github("bit2r/remotes")
library(bitTA)  # 라이브러리 호출 import / 실행 한 번 해보고 없으면 아래를 설치 / You can install it with install_mecab_ko(). 설치 하기
bitTA::install_mecab_ko()  # 한국어 형태소 사전 및 형태소 분석기를 설치   # mecab: 이 라이브러리를 쓰려면 자바가 필요함.
install.packages("RcppMeCab")

# NLP 연구자들이 가장 많이 사용하는 라이브러리 3개
library(bitTA)
library(tidyverse)
library(tidytext)

# 실습: 한국어 띄어쓰기
# 띄어쓰기를 하는 이유: 조사 / 띄어쓰기를 안 하면 내가 원하는 포인트를 찾을 수 없음.
get_spacing("최근음성인식정확도가높아짐에따라많은음성데이터가텍스트로변환되고분석되기시작했는데,이를위해잘동작하는띄어쓰기엔진은거의필수적인게되어버렸다") # 조사로만 띄어쓰기 한 경우 / 100/70 / 조사를 찾는 것 까지는 확인이 됨.
