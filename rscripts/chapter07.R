...
# 실습: 실습용 데이터 가져오기 
getwd()
setwd("C:/Rwork/Part-II")  # 데이터를 다운받아서 설정하기
dataset <- read.csv("dataset.csv", header = T)
dataset

# 실습: 전체 데잍 보기 
print(dataset)
View(dataset)  # 별도의 창이 생성됨.

# 실습: 데이터의 앞부분과 뒷부분 보기 - 데이터의 형태를 파악할 수 있음.
head(dataset)
tail(dataset)
...
dataset[2]  # 2번 째 컬럼을 의미
dataset[6]
dataset[3, ]  # [3번째 레코드, 전체]
dataset[ , 3]  # x는 전부 다 가져오기
...
# 실습: 범주형 변수의 극단치 처리하기 
table(dataset$gender)  # table이라는 용어가 헷갈림.
pie(table(dataset$gender))


# 실습: subset() 함수를 사용하여 데이터 정제하기 
dataset <- subset(dataset, gender == 1 | gender == 2)  # () 를 뽑아서, 뽑는다는 것이 subset을 의미
dataset
length(dataset$gender)
pie(table(dataset$gender))
pie(table(dataset$gender), col = c("red", "blue"))
# R의 특징: 데이터의 분포를 그래프로 그려준다.
# 박스, pie... -> 가시화
# 데이터를 파악하기에 좋다.
...
barplot(resident_gender, beside = T, 
        col = rainbow(5), 
...
# 단계 1: 데이터 파일 가져오기 
pay_data <- read.csv("pay_data.csv", header = TRUE, fileEncoding = "CP949", encoding = "UTF-8")
head(pay_data, 10)
...
setwd("C:/Rwork/Part-II")
write.csv(user_pay_data, "cleanData.csv", quote = F, row.names = F, fileEncoding = "CP949", encoding = "UTF-8")
...
training <- iris[idx, ]  # 70프로
testing <- iris[-idx, ]  # 70프로를 뺀 나머지
dim(training)
...
# 랜덤하게 가져오기 때문에 값이 실행할 때 마다 달라질 수 있어
